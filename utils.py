...
# --- PPO validation imports (kept optional-ish to avoid affecting DQN-only users) ---
from ppo.PPO_actor_critic import Actor as PPOActor
...
def run_loaded_ppo_model_on_route(
    model_path: str,
    route_id: str,
    use_gui: bool = False,
    max_steps: int | None = None,
    *,
    ...
    """Load a trained PPO actor checkpoint and run one evaluation episode on a chosen route.

    The PPO checkpoint is expected to be the dict saved by `ppo/train_ppo.py` with at
    least the key `actor_state_dict`.
    ...
    if not isinstance(checkpoint, dict) or "actor_state_dict" not in checkpoint:
        raise ValueError(
            f"PPO checkpoint at {model_path!r} is expected to be a dict with key 'actor_state_dict'."
        )

    obs_size = 12  # keep in sync with sumo_utils.get_state()
    n_actions = len(Action)

    actor = PPOActor(obs_size, n_actions).to(DEVICE)
    ...
    for step in range(max_steps):
        ...
        # Incident accounting (de-duplicated, same as DQN validation)
        collision_events = traci.simulation.getCollisions()
        teleport_ids = traci.simulation.getStartingTeleportIDList()
        emergency_ids = traci.simulation.getEmergencyStoppingVehiclesIDList()
...
def validate_ppo_model_on_routes(
    *,
    model_path: str,
    sumo_config: str,
    traffic_scales: list[float] | tuple[float, ...] = (2, 3, 5),
    ...
    """Run PPO validation across all `config.VALIDATION_ROUTES` and traffic scales.

    Output TSV format matches `validate_model_on_routes()` so you can compare runs.
    ...
    for scale in traffic_scales:
        for route_id in config.VALIDATION_ROUTES:
            combo_episode += 1
            reward, steps, end_reason, metrics = run_loaded_ppo_model_on_route(
                model_path,
                route_id,
                use_gui=use_gui,
                max_steps=max_steps,
