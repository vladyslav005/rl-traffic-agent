...
def watch_checkpoint(checkpoint_path, route_id, sleep_s=0.03, deterministic=True, after_spawn_wait=2.0):
    ...
    # ВАЖНО: reset_sumo() здесь НЕ вызываем.
    # В твоём проекте reset_sumo() закрывает текущий TraCI/SUMO процесс,
    # поэтому GUI завершался сразу с сообщением: "TraCI requested termination".
    # Для одного episode SUMO уже запущен через start_sumo_gui(),
    # поэтому просто добавляем ego vehicle и начинаем симуляцию.
    spawn_ego(route_id)
